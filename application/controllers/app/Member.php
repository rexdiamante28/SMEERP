...
class Member extends CI_Controller {
	...
	public function view()
	{
        ...
        if($this->loginstate->get_access()['overall_access']==1)
        {
            ...
            $forms = array(
            	//$this->load->view('app/member/form_view','',true)
            );

            $data = array(
                'view' => $this->load->view("app/member/table_view",$sub_data,true),
                'title' => $page_title,
                'add_css' => array(),
                'add_js' =>  array('assets/js/app/member/create.js','assets/js/app/member/main.js'),
                'forms' => $forms
            );
	...
	public function list()
	{
		if($this->loginstate->get_access()['overall_access'] == 1)
		{
        	...
        	$result = $this->model_member->list($read_args);
        	...
        	$nestedData = array();
            $nestedData[] = '
                <img style="height:30px;" class="img img-circle" src="'.base_url('assets/uploads/avatars/').$row['avatar'].'">
            ';
        	$nestedData[] = $row["name"];
