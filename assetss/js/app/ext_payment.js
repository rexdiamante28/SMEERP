$(document).ready(function(){
	...
	//payment variables
	merchant_id = $('#ext_merchant_id').val();
	reference_number = $('#ext_si_number').val();
	service_id = 0;
	payment_choice = "";
	payer_email = $('#ext_email_address').val();
	payer_name = "";
	amount_to_pay = 0.00;
	currency = "";
	remarks = "";
	...
	function initiate_payment()
	{
		...
		formData.append('merchant_id',merchant_merchant_id);
	...
	get_merchant_details = function(merchant_id)
	{
		showCover("Loading data...");

		$.ajax({
		  		type: 'get',
		  		url:  base_url+'app/merchant/get_merchant_details/'+merchant_id,
		  		data: '',
		  		contentType: false,   
				cache: false,
				processData:false,
		  		success:function(data){
		  			...
		  			$('#i_merchant_name').html(json_data.message.merchant_name);
		  			$('#pp_partner').html(json_data.message.merchant_name);
		  			merchant_merchant_id = json_data.message.merchant_id;
	...
	get_merchant_details(merchant_id);


	fill_form = function()
	{
		var remarks = "Payment to "+$('#i_merchant_name').html();
		$('#m_reference_text').val(reference_number);
		$('#m_email_address').val(payer_email);
		$('#m_remarks').val(remarks);
		...
		if($('#m_reference_text').val()!='')
		{
			...
			$.ajax({
		  		type: 'get',
		  		url:  base_url+'app/merchant/get_ref_amount/'+merchant_merchant_id+'/'+ref,
		  		data: '',
		  		contentType: false,  
				cache: false,
				processData:false,
			...
			$.ajax({
				...
				processData:false,
		  		success:function(data){
		  			...
		  			$('input[name='+ajax_token_name+']').val(ajax_token);

		  			$('#manual_reference_form').submit();
